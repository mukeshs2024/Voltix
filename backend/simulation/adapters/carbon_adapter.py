...
class CarbonAdapter:
    async def process(self, payload: SimulationOperatorInput) -> CarbonAgentResponse:
        ...
        metadata = AgentDeveloperMetadata(
            agent_name="Carbon Agent",
            source="backend.simulation.adapters.carbon_adapter",
            raw_ai_response=ai_result,
            normalized_at=datetime.now(timezone.utc),
        )

        return CarbonAgentResponse(
            agent_id=payload.agent_id,
            input=payload,
            workflow=workflow,
