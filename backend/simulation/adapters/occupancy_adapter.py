...
class OccupancyAdapter:
    async def process(self, payload: SimulationOperatorInput) -> OccupancyAgentResponse:
        ...
        metadata = AgentDeveloperMetadata(
            agent_name="Occupancy Agent",
            source="backend.simulation.adapters.occupancy_adapter",
            raw_ai_response=ai_result,
            normalized_at=datetime.now(timezone.utc),
        )

        return OccupancyAgentResponse(
            agent_id=payload.agent_id,
            input=payload,
            workflow=workflow,
