"""
1. Purpose: Observability and Metrics for the Safety Agent.
...
logger = logging.getLogger("Voltix.SafetyAgent")


class SafetyAgentMetrics:
    ...
    @classmethod
    def record_blocked(cls, count: int):
        cls.validations_blocked += count
    ...
    def get_summary(cls) -> dict:
        ...
        return {
            ...
            "validations_blocked": cls.validations_blocked,
        }
    ...
    def wrapper(*args, **kwargs) -> Any:
        ...
        try:
            ...
            if isinstance(result, dict) and result.get("errors"):
                SafetyAgentMetrics.record_fallback(exec_time_ms)
                logger.warning(
                    f"Safety pipeline finished with fallback/errors in {exec_time_ms:.2f}ms."
                )
            else:
                SafetyAgentMetrics.record_success(exec_time_ms)
                logger.info(
                    f"Safety pipeline completed successfully in {exec_time_ms:.2f}ms."
                )
            return result
            ...
            SafetyAgentMetrics.record_fallback(exec_time_ms)
            logger.error(
                f"Safety pipeline suffered unhandled crash in {exec_time_ms:.2f}ms. Error: {exc}",
                exc_info=True,
            )
            raise exc
