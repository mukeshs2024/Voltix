...
default_zone = ZoneTopology(
    zone_id="z_01", name="Main Office", capacity=100, sq_ft=5000.0
)
...
SCENARIOS = {
    "EMPTY_OFFICE": SharedState(
        ...
        sensors=[
            SensorData(
                sensor_id="s1", sensor_type=SensorType.PIR, value=0, timestamp=now
            ),
            SensorData(
                sensor_id="s2", sensor_type=SensorType.CO2, value=400, timestamp=now
            ),
        ],
        calendar=[],
    ),
    "MORNING_RUSH": SharedState(
        ...
        sensors=[
            SensorData(
                sensor_id="s1", sensor_type=SensorType.ACS, value=45, timestamp=now
            ),
            SensorData(
                sensor_id="s2", sensor_type=SensorType.PIR, value=1, timestamp=now
            ),
        ],
        calendar=[],
    ),
    "MOTION_ONLY": SharedState(
        zone=default_zone,
        sensors=[
            SensorData(
                sensor_id="s1", sensor_type=SensorType.PIR, value=1, timestamp=now
            ),
            SensorData(
                sensor_id="s2", sensor_type=SensorType.CO2, value=410, timestamp=now
            ),
        ],
        calendar=[],
    ),
