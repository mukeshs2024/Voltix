...
class AgentMetrics:
    ...
    def get_summary(cls) -> dict:
        avg_time = (cls.total_execution_time_ms / cls.total_invocations) if cls.total_invocations else 0.0
        return {
    ...
    def wrapper(*args, **kwargs) -> Any:
        ...
        try:
            ...
            if isinstance(result, dict) and "errors" in result and result["errors"]:
                ...
                logger.warning(f"Pipeline finished with fallback/errors in {exec_time_ms:.2f}ms.")
            else:
            ...
            exec_time_ms = (time.perf_counter() - start_time) * 1000
            AgentMetrics.record_fallback(exec_time_ms)
            logger.error(f"Pipeline suffered unhandled crash in {exec_time_ms:.2f}ms. Error: {e}", exc_info=True)
            raise e
