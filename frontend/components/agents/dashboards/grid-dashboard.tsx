...
import { GridAgentResponse } from "@/types/agent-workbench";
import { Zap, Activity } from 'lucide-react';

export default function GridDashboard({ data }: { data: GridAgentResponse }) {
    return (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div className="bg-slate-900/50 border border-slate-800 rounded-xl p-6">
                <div className="flex items-center gap-2 mb-4">
                    <Zap className="w-5 h-5 text-slate-400" />
                    <h3 className="text-sm font-medium text-slate-300 uppercase tracking-wider">Grid Telemetry</h3>
                </div>
                        ...
                        <tbody className="divide-y divide-slate-800/50">
                            {data.sensors?.map((s: any, i: number) => (
                                <tr key={i}>
                                    ...
                                    <td className={`py-3 text-right font-medium ${s.status === 'critical' ? 'text-amber-400' : 'text-slate-200'}`}>
                                        {s.value}
                                    </td>
                                </tr>
                            ))}
            ...
            <div className="bg-slate-900/50 border border-slate-800 rounded-xl p-6">
                <div className="flex items-center gap-2 mb-4">
                    ...
                    <h3 className="text-sm font-medium text-slate-300 uppercase tracking-wider">Grid Analytics</h3>
                </div>
